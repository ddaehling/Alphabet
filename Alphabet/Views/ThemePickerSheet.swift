            ...
            ScrollView {
                ...
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    Text("Speaks each letter aloud as it's tapped, with a short pause so the child can hear it before the next one.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                    Toggle("Real voice for words", isOn: $store.useOnlinePronunciation)
                        .font(.system(size: 17, weight: .semibold, design: .rounded))
                    Text("Downloads a real human recording for each whole word (from a free online dictionary) and saves it on the iPad, so it plays instantly and offline next time. Needs Wi-Fi the first time a word is heard; falls back to the built-in voice otherwise.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
